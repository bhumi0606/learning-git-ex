...
async def deposit(user_id,amount,description,session):
...
async def withdraw(user_id,amount,description,session):
    acc = await session.query(Account).filter_by(user_id=user_id).one_or_none()
    acc.balance -= amount
    await session.commit()
    transaction = Transaction(
...
async def transfer(from_account,to_account,amount,description,session):
    from_acc = await session.query(Account).filter_by(account_number=from_account).one_or_none()
    to_acc = await session.query(Account).filter_by(account_number=to_account).one_or_none()
...
async def get_transaction(sort_by,search_by,filter_by,max_amount,min_amount,session):
    query = await session.query(Transaction)
